...
const positiveNotification = chalk.bgHex('#6aa84f').black, negativeNotification = chalk.bgHex('#e14747').black, heading = chalk.bgHex('#3d85c6').hex('#000000');
export let stdntIdies = [];
let stdntAllCourses = [{ ID: '', Name: '', All_Courses: [] }];
const courses = { Arabic: 750, English: 350, German: 550, Italian: 400, Persian: 650 }, studentDatabase = [];
const showStdntData = (id) => {
...
const selectStudent = async (toPay, forStatus) => {
    let stdntSelected, selectStdntAgain, quiteCase = false, firstIteration = true;
    if (toPay === true) {
    ...
    switch (mainMenu.option) {
        ...
        case 'Certify Student':
            ...
            if (stdntDetais.Dues === 'Paid' && allCousesStdntObjt !== undefined) {
                ...
                let noDuplicateCourses = allCourses.filter((item, index) => {
